...
int main() {
	...
	do {
		std::vector<int> v;
		const int accuracy = utils::read_int("Enter accuracy: ", 1, 5000);//êîëè÷åñòâî âûïîëíåíèÿ ñîðòèðîâêè äëÿ âåêòîðîâ îäíîãî è òîãî æå ðàçìåðà 		
		std::vector<int> SIZES;//Âåêòîð ðàçìåðîâ
		...
		std::vector<int> vcopy = v;//ñîçäàåì êîïèþ âåêòîðà 
		system("pause");
		int User_Choise2 = 1;
		std::vector<double> time;//âåêòîð âðåìåí, çà êîòîðûå âû÷èñëÿåòñÿ îïðåäåëåííàÿ ñîðòèðîâêà ñ âåêòîðàìè îäíîãî è òîãî æå ðàçìåðà

		double average_time = 0.0;//Ñðåäíåå âðåìÿ âûïîëíåíèÿ îäíîé ñîðòèðîâêè ñ âåêòîðàìè îäíîãî è òîãî æå ðàçìåðà
		std::vector<double> Average_Times;//Âåêòîð ñðåäíèõ âðåìåí êàæäîé ñîðòèðîâêè ñî âåêòîðàìè ðàçíûõ ðàçìåðîâ
		do {
			system("cls");
			...
			case 0:
				...
				for (size_t i = 0; i < number_sizes; ++i) {
					...
					average_time = average_time / accuracy;//ïîëó÷èëè ñðåäíåå âðåìÿ âûïîëíåíèÿ ñîðòèðîâêè ñ âåêòîðàìè îäíîãî ðàçìåðà
					time.clear();
